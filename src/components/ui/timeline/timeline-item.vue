...
const position = computed(()=> props.position ==='end'? 'timeline-end ': 'timeline-start text-end')
</script>
<template>
<li>
   <slot />
    ...
    <div :class="['md:mb-10',position]">
      <time class="font-semibold text-primary italic block">{{ event.time }}</time>
      <p class="text-sm inline-flex items-center gap-1"><AppIcon icon="ic:round-location-on" size="12"/> {{ event.location }}</p>
